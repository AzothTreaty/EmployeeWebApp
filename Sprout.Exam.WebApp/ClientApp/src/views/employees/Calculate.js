...
export class EmployeeCalculate extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { id: 0,fullName: '',birthdate: '',tin: '',typeId: 1,absentDays: 0,salaryAmount: 0,workedDays: 0,netIncome: 0, loading: true,loadingCalculate:false };
  }
...
{ this.state.typeId === 1?
                    <div className="form-row">
                        <div className='form-group col-md-12'><label>Salary: <b>{this.state.salaryAmount}</b> </label></div>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async getEmployee(id) {
    ...
    if(response.status === 200){
        const data = await response.json();
        this.setState({ id: data.id,fullName: data.fullName,birthdate: data.birthdate,salaryAmount: data.salaryAmount,tin: data.tin,typeId: data.typeId, loading: false,loadingCalculate: false });
    }
    else{
